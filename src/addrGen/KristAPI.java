package addrGen;
import addrGen.types.Address;
import addrGen.types.Transaction;
import addrGen.util.HTTP;
// import addrGen.util.SHA256;
...
public class KristAPI {
	...
	public static String makeAddressV2(String key) {
		...
		int link = 0;
		String v2 = "k";
		//Inner hash = thash and output
		//Outer hash = thash2 and op2, stick is hashed once more.
		String stick = DigestUtils.sha256Hex(DigestUtils.sha256Hex(DigestUtils.sha256Hex("KRISTWALLET" + key) + "-000"));
		for (int i = 0; i <= 9; i++) {
			if (i < 9) {
